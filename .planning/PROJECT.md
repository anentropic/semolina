...
- **Warehouse type fidelity.** ✓ Measured and decided in Phase 47. The comparison is generated by `just type-fidelity`, committed, and guarded against drift; `47-DECISIONS.md` is normative for Phases 48 and 50. Settled: warehouse decimals map to `decimal.Decimal` on all three backends (including Snowflake's whole `FIXED` family at scale 0), metric annotations are uniformly `T | None` with COUNT a documented over-approximation, and the query-time result schema is primary with warehouse metadata a labelled fallback. The policy is **annotation-only** — `cursor.py:281` already returns `Decimal` via `to_pylist()`, so no runtime coercion is implied. Still to implement: the category-1 map gaps (DuckDB `DECIMAL`/`UUID`/`JSON`/`ENUM`/`TIMESTAMP_S|MS|NS`, Databricks `interval`).
