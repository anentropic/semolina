...
- [x] **TYPE-03**: User generating models for decimal-typed warehouse columns gets the type the decision doc specifies, applied consistently across Snowflake, Databricks, and DuckDB — the three backends no longer disagree about money. *(⚠️ **The trailing clause is falsified as literally worded, measured 2026-08-16, and the wording needs a maintainer's decision — see below.** Snowflake and DuckDB annotate `decimal.Decimal`; Databricks annotates `str`, because the Foundry ADBC driver returns every decimal as an Arrow string at any precision and scale, scale 0 included. The rule Decision 1 states — annotate what the driver returns — is unchanged and is what produces the divergence; Decision 1's own premise, "a user with a money column already receives a Decimal today", was measured on DuckDB and Snowflake and is false on the backend it never measured. Left ticked because the defect TYPE-03 was written against is fixed and stays fixed: before Phase 48 the three backends gave three arbitrary answers (`int`/`float`, `float`, `TODO:`) and none described the value that arrived, whereas now each names what its own driver delivers and no generated model lies about money. What is no longer true is the uniformity, which was evidence for that property rather than the property itself. The open question is whether to reword TYPE-03 to say so, or reopen it — a roadmap call, not a code one. `tests/unit/codegen/test_type_map.py::test_decimal_annotation_follows_each_driver` now pins the measured state either way, including that no backend emits a `TODO:`.)*
...
| TYPE-03 | Phase 48 | Complete — ⚠️ wording needs review: "the three backends no longer disagree about money" is falsified as written (Databricks annotates `str`, measured 2026-08-16); the substance holds, the uniformity claim does not |
