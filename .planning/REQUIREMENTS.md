...
- [ ] **DTO-09**: User running codegen against a driver that does not implement `adbc_execute_schema` gets a working fallback (zero-row execution) rather than a hard failure. *(Partial after Phase 50: proved as a branch, not as a backend. The fallback fires on a live DuckDB cursor whose `adbc_execute_schema` is monkeypatched to refuse, and the class it yields is identical field for field to the primary route's and round-trips through `.into()`. But the only driver that genuinely does not implement `adbc_execute_schema` is the Foundry ADBC Databricks driver, and that route has never been run: `pytest-adbc-replay` structurally cannot settle it — it serves `adbc_execute_schema` from the recorded result table regardless of driver capability, so a cassette-backed test would look like evidence and be none. RESEARCH assumption A2, that Databricks accepts `SELECT * FROM (<MEASURE… GROUP BY ALL>) WHERE 1=0` at all, is unmeasured; if it is false, DTO codegen has no working route on that backend rather than a slower one. Follows the TYPE-05 precedent rather than being ticked because the phase is closing. Tracked as WINDOWS.md entry 12; closing it needs `.planning/todos/pending/2026-08-12-verify-databricks-zero-row-fallback.md` and a live workspace.)*
...
| DTO-09 | Phase 50 | Partial — fallback branch proved on a DuckDB cursor made to refuse; Databricks, the only backend that takes it, unverified and RESEARCH A2 unmeasured (WINDOWS 12, needs a live workspace) |
