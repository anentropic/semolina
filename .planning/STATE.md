...
- [R-04, 2026-08-16]: SETTLED — do not narrow. Decision 2's conclusion stands, its rationale is replaced by a dated correction in 47-DECISIONS.md. Settled as a standalone decision, not a phase: the outcome needed no scheduled work, so no code, snapshot, docs sample or user DTO moves. Two things decided it. (1) The error is asymmetric and the dangerous direction is uncatchable — dto.py strips NoneType and compares base types only (D-09), so an over-narrow annotation fails when a NULL arrives in a running service, never at .into(); an unrecognised COUNT costs an is-None check nobody needed. (2) The cheap alternative was MEASURED DEAD: every probed field reads nullable=True including the COUNT, and the Arrow type does not separate n_order_totals (COUNT) from total_order_count (SUM over int) — both int64 — so there is no partition to read. Also recorded: the surviving option costs more than the todo estimated, since codegen-dto calls no introspect() at all. COUNT_IF's empty-group behaviour was deliberately NOT measured; it is the place to start if anyone reopens this. docs/src/explanation/type-fidelity.rst rewritten, since it stated the dissolved reason to users.
...
- ✅ **R-04 settled 2026-08-16 without a phase** — the one open decision that needed no warehouse. `47-DECISIONS.md` no longer carries a rejection resting on a dissolved premise, and the todo is retired to `todos/completed/`. Nothing shipped changes.
