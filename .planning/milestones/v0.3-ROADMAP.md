...
- 🚧 **v0.3 Arrow & Connection Layer** — Phases 25-32 (in progress)
...
- [x] **Phase 25: Pool Registry, Dialect Enum & MockPool** - Replace Engine registry with pool+dialect registry; MockPool enables testing without warehouse (completed 2026-03-16)
- [x] **Phase 26: SemolinaCursor & Row Convenience** - `.execute()` returns SemolinaCursor with Row convenience methods; Result class removed (completed 2026-03-17)
- [x] **Phase 27: TOML Configuration & Real Pools** - `.semolina.toml` config loading into adbc-poolhouse config classes; `pool_from_config()` helper (completed 2026-03-17)
- [x] **Phase 28: Query Shorthand** - `query(metrics=..., dimensions=...)` keyword args additive with builder methods (completed 2026-03-17)
- [x] **Phase 29: Documentation Update** - Update tutorials, how-to guides, and API reference for v0.3 API changes (completed 2026-03-17)
...
Plans:
- [x] 25-01-PLAN.md — Dialect StrEnum, resolve_dialect(), registry rewrite for pool+dialect storage
- [x] 25-02-PLAN.md — MockPool/MockConnection/MockCursor, wire _Query.execute() to pool registry

### Phase 26: SemolinaCursor & Row Convenience
**Goal**: `.execute()` returns a SemolinaCursor providing Row convenience methods; Result class removed entirely
...
Plans:
- [x] 26-01-PLAN.md — SemolinaCursor class with Row convenience methods, MockCursor DBAPI compliance
- [x] 26-02-PLAN.md — Wire execute() to return SemolinaCursor, remove Result class, update all tests
...
**Success Criteria** (what must be TRUE):
  1. `.semolina.toml` uses flat `[connections.name]` sections with a `type` field (e.g. `type = "snowflake"`) that determines which adbc-poolhouse config class to use
  2. TOML connection settings load into the correct adbc-poolhouse config class (SnowflakeConfig or DatabricksConfig) via direct tomllib parsing and dict unpacking
  3. User can call `pool_from_config(connection="conn1")` and receive a `(pool, Dialect)` tuple ready for `register()`, defaulting to `.semolina.toml` path
**Plans**: 2 plans

Plans:
- [x] 27-01-PLAN.md — adbc-poolhouse dependency, config.py with pool_from_config() and tests (TDD)
- [x] 27-02-PLAN.md — Public API export, .semolina.toml.example update, registry.reset() close_pool
...
Plans:
- [x] 28-01-PLAN.md — TDD: tests for shorthand/additivity, implement query() kwargs in models.py
...
Plans:
- [x] 29-01-PLAN.md — Rewrite tutorial (first-query.md) and homepage for v0.3 API
- [x] 29-02-PLAN.md — Rewrite backend how-to guides (overview, snowflake, databricks) for pool_from_config and manual pool construction
- [x] 29-03-PLAN.md — Update queries how-to with SemolinaCursor/shorthand, minor page updates, stale-term sweep, API reference verification
...
| 25. Pool Registry, Dialect & MockPool | v0.3 | 2/2 | Complete | 2026-03-17 |
| 26. SemolinaCursor & Row | v0.3 | 2/2 | Complete | 2026-03-17 |
| 27. TOML Config & Real Pools | v0.3 | 2/2 | Complete | 2026-03-17 |
| 28. Query Shorthand | v0.3 | 1/1 | Complete | 2026-03-17 |
| 29. Documentation Update | v0.3 | 3/3 | Complete | 2026-03-17 |
| 30. Sphinx Shibuya Migration | v0.3 | 4/4 | Complete | -- |
| 31. Fix MockPool Doc Example | v0.3 | 1/1 | Complete    | 2026-04-18 |
| 32. v0.3 Tech Debt Cleanup | v0.3 | 1/1 | Complete    | 2026-04-18 |

### Phase 30: Sphinx Shibuya Documentation Migration

**Goal:** Replace MkDocs Material with Sphinx + shibuya theme. All 34 markdown docs convert to RST. API reference generated by sphinx-autoapi. No content changes.
**Requirements**: SPHINX-01, SPHINX-02, SPHINX-03, SPHINX-04
**Depends on:** Phase 29
**Success Criteria** (what must be TRUE):
  1. `sphinx-build -W` exits 0 with no warnings
  2. All 16 content pages render correctly in the built site
  3. sphinx-autoapi generates reference for all public modules
  4. Tab sync works across Snowflake/Databricks tab groups
  5. All `:doc:` and `:ref:` cross-references resolve
  6. CI workflow builds and deploys correctly
**Plans:** 4/4 plans complete

Plans:
- [x] 30-01-PLAN.md — Sphinx scaffold & configuration (conf.py, deps, delete mkdocs artifacts)
- [x] 30-02-PLAN.md — Convert tutorials & explanation pages to RST (5 pages)
- [x] 30-03-PLAN.md — Convert how-to guides & homepage to RST (11 pages, 49 tab blocks)
- [x] 30-04-PLAN.md — API reference tuning, CI updates, skill updates, cleanup

### Phase 31: Fix MockPool Documentation Example
**Goal:** Fix incorrect `test_filtered_query()` example in warehouse-testing docs so it matches MockPool behavior
**Depends on:** Phase 30
**Requirements:** DOCS-03
**Gap Closure:** Closes gaps from v0.3 audit (DOCS-03 partial, integration gap, broken flow)
**Success Criteria** (what must be TRUE):
  1. `warehouse-testing.rst` `test_filtered_query()` example has correct assertion matching MockPool behavior
  2. Example narrative is consistent with surrounding documentation
  3. `sphinx-build -W` passes with no warnings
**Plans:** 1/1 plans complete

Plans:
- [x] 31-01-PLAN.md — Rewrite test_filtered_query section with correct assertions and narrative

### Phase 32: v0.3 Tech Debt Cleanup
**Goal:** Clean up deprecated API usage and stale "engine" terminology flagged by milestone audit
**Depends on:** Phase 25
**Requirements:** None (tech debt)
**Gap Closure:** Closes tech debt from v0.3 audit
**Success Criteria** (what must be TRUE):
  1. `src/semolina/conftest.py` uses `register()` with `dialect=` parameter (no deprecation warning)
  2. `src/semolina/query.py` `using()` docstring says "pool" not "engine"; parameter renamed from `engine_name`
  3. `tests/unit/test_query.py` test classes use current `register(name, pool, dialect=)` API
  4. All existing tests pass
**Plans:** 1/1 plans complete

Plans:
- [x] 32-01-PLAN.md — Convert MockEngine to MockPool in conftest/tests, rename engine_name to pool_name in query.py
