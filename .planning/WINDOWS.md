...
fixed_count: 3
total_count: 11
last_updated: 2026-08-14T07:24:23.117Z
---
...
| 3 | 47 | deviation | .planning/phases/47-type-fidelity-probe-decision-doc/47-TYPE-FIDELITY.md |  | The artifact's Downstream Decimal pandas row is environment-dependent: pandas is not a declared dependency and arrives only via databricks-sql-connector[pyarrow] under the 'all' extra. CI syncs --dev --extra all so the row measures there, but regenerating after a plain 'uv sync --dev' flips it to 'not measured' and turns test_committed_table_is_not_stale red for an environment reason rather than genuine drift. Documented in the section text; not fixed. The user-facing half of this is now documented rather than only recorded: 48-06 wrote the TIMESTAMP_NS environment dependence (a pandas.Timestamp when pandas is importable; microsecond truncation and a ValueError on sub-microsecond input when it is not) into docs/src/explanation/type-fidelity.rst per D-04. The artifact's regeneration hazard is unfixed. Correction to the command named above: regenerate under 'uv sync --all-groups --extra all', not 'uv sync --dev --extra all', which prunes the docs dependency group and breaks just docs-build (48-01). CLOSED 2026-08-14 by Phase 49 plan 03. The 'not a declared dependency' premise is gone: Phase 49 plan 01 published a [pandas] extra and put it inside [all] (D-12), and plan 03 regenerated the artifact so its own caveat paragraph names the extra as the route rather than databricks-sql-connector[pyarrow]. What remains, stated plainly in the regenerated prose, is that a sync omitting --extra all still flips the row to 'not measured' — but that is now a declared install choice a reader can act on, not an undeclared transitive accident. The same sentence now covers polars, which the [polars] extra put in exactly the same position. | fixed |  | 2026-08-12T00:20:32.562Z | 2026-08-14T07:24:12.941Z |
...
| 10 | 49 | unrun-verify | .planning/phases/47-type-fidelity-probe-decision-doc/47-TYPE-FIDELITY.md |  | type_fidelity_probe.py --check is red on the polars and pydantic rows after the pydantic 2.12.5->2.13.4 bump; Plan 03 (D-16) owns regenerating it and must edit _measure_polars, not just the artifact CLOSED 2026-08-14 by Phase 49 plan 03 (D-16). _measure_polars was rewritten to take the probe table and really measure — polars 1.43.2 maps decimal128(38,2) to a native Decimal(precision=38, scale=2) dtype holding decimal.Decimal — and the generated closing prose in render_downstream_decimal was updated in the same change, so the artifact was regenerated from a corrected generator rather than hand-edited. --check exits 0 and test_committed_table_is_not_stale is green. | fixed |  | 2026-08-14T07:19:00.152Z | 2026-08-14T07:24:23.117Z |
...
[
  ...
  },
  {
    ...
    "file": ".planning/phases/47-type-fidelity-probe-decision-doc/47-TYPE-FIDELITY.md",
    "line": null,
    "description": "type_fidelity_probe.py --check is red on the polars and pydantic rows after the pydantic 2.12.5->2.13.4 bump; Plan 03 (D-16) owns regenerating it and must edit _measure_polars, not just the artifact CLOSED 2026-08-14 by Phase 49 plan 03 (D-16). _measure_polars was rewritten to take the probe table and really measure \u2014 polars 1.43.2 maps decimal128(38,2) to a native Decimal(precision=38, scale=2) dtype holding decimal.Decimal \u2014 and the generated closing prose in render_downstream_decimal was updated in the same change, so the artifact was regenerated from a corrected generator rather than hand-edited. --check exits 0 and test_committed_table_is_not_stale is green.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-08-14T07:19:00.152Z",
    "resolved_at": "2026-08-14T07:24:23.117Z"
  },
  {
    "id": 11,
    "kind": "deviation",
